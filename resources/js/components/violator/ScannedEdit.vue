...
<div>
    <div class="card">
        <div class="card-header">
            <i class="nav-icon fa fa-plus"></i> Edit Scanned Violator
        </div>
            ...
            <form autocomplete="off" v-on:submit="saveForm()">
                <div class="form-row">
                    ...
                    <div class="form-group  col-md-4">
                        ...
                        <input type="text" class="form-control" v-on:keyup="showMembers" v-model="data.plate_no" readonly>
                        <input type="hidden" v-model="data.vehicle_id">
                    </div>
                    <div class="form-group  col-md-2">
                        <label for="exampleFormControlInput1">Date Violated</label>
                        <input type="date" class="form-control" v-model="data.date_violated" readonly>
                    </div>
                    <div class="form-group  col-md-2">
                        <label for="exampleFormControlInput1">Time Violated</label>
                        <input type="time" class="form-control" v-model="data.time_violated" readonly>
                    </div>
                    <div class="form-group  col-md-12">
                        <label for="exampleFormControlInput1">Violation</label>
                        <select class="form-control" v-model="data.violation">
                            <option v-for="violation in violations" :key="violation.id" v-bind:value="violation.id" readonly>{{ violation.title }}</option>
                        </select>
                    </div>
                    <div class="form-group col-md-12">
...
export default {
    ...
    computed: {
        formatDate(oldFormat){
            var dt = new Date(oldFormat);
            return dt;
        },
    ...
    methods: {
        saveForm() {
            ...
            axios
                .post("/violator/scanned/update", data)
                .then(function (resp) {
                    console.log(resp);
                    // if (resp.data == 'failed') {
                    //     alert("Could not Register: License number exist!");
                    // }
                    app.$router.push("/violator/scanned/list");
                })
                .catch(function (resp) {
                    console.log(resp);
